...
func (s *calendar) Next(t time.Time) time.Time {
	...
	for i := 0; i < maxSearchDays; i++ {
		day := time.Date(y, m, d+i, 0, 0, 0, 0, s.loc)
...
// firstOn returns the first scheduled time on day that is not before
// start. A time that does not exist on that day, such as one skipped by
// a change to daylight saving time, moves to the time that the clock
// shows instead.
func (s *calendar) firstOn(day, start time.Time) (time.Time, bool) {
	y, m, d := day.Date()
	sy, sm, sd := start.Date()
	h0, m0, s0 := 0, 0, 0
	sameDay := y == sy && m == sm && d == sd
	if sameDay {
		h0, m0, s0 = start.Clock()
	}
	for h := h0; h < 24; h++ {
		...
		for mi := minFrom; mi < 60; mi++ {
			...
			for se := secFrom; se < 60; se++ {
				...
				c := time.Date(y, m, d, h, mi, se, 0, s.loc)
				// The clock skips this time where daylight saving time
				// starts. Depending on the zone, time.Date then returns a
				// time before or after the gap; keep the one after it.
				want := clockOf(h, mi, se)
				if ch, cm, cs := c.Clock(); clockOf(ch, cm, cs) < want {
					c = c.Add(want - clockOf(ch, cm, cs))
				}
				if c.Day() == d && !c.Before(start) {
					return c, true
				}
			}
		}
