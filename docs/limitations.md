...
- **Untested service setups**: the tests run on Linux, macOS and Windows for every change, but the service setup on Linux and Windows, and the LaunchDaemon setup on macOS, have not yet been tried on real machines. The LaunchAgent setup on macOS has been.
